...
for j in range(0,len(major_seq_train)): #eachset
    ...
    for eachseq in apples: #each sequence in the set
        ...
        a =[eachseq[i:i+5] for i in range(len(eachseq)-4)]
...
for j in one_seq_test:
    ...
    for eachseqlist in pears: #eachseq as set
        for seq in eachseqlist:
            p =[seq[i:i+5] for i in range(len(seq)-4)]
            seq_test.append(p)
    # print(seq_test)
...
standardwhole=clf.score(mappedflatten_settestmap,fea_test_setarray)
print(standardwhole)
